...
本标准并未规定对非规格化浮点数的算术运算的行为，也没有规定何时或是否应该创建这样的表示。这与 IEC 60559:1989 保持一致。此外，本标准并未规定如何访问创建的 **NaN** 的确切位模式，以及在 32 位和 64 位表示之间转换 **NaN** 时的行为。所有这些行为都被有意地留给实现特定。
 ...
 * 非托管指针应被视为无符号 (即，使用 `conv.ovf.u` 而不是 `conv.ovf.i` 等)。
 ...
 * 如果你通过引用传递一个参数，相应的参数就是一个托管指针。
 * 托管指针不能存储在静态变量、数组元素或对象或值类型的字段中。(C# 除 *ref-struct* 44类型以外的值类型。) 
 * 托管指针不可以与对象引用互换。
...
方法调用涉及到在堆栈上的参数值被隐式地分配给被调用方法的参数（使用 `ldarg` 或 `ldarga`，指令访问）。这个分配是一个隐式的 `starg` 指令，可以被称为隐式参数强制。在验证的 CLI 中，隐式参数强制的有效性，就像 `starg` 指令一样，由验证器可分配关系（§[III.1.8.1.2.3](iii.1.8.1.2.3-verification-type-compatibility.md)）确定。正确的 CIL 也允许一个 `native int` 作为一个 **byref**（`&`）传递；在这种情况下，随后的隐式转换后，该值将被垃圾收集跟踪。
...
<sup>1</sup> 签名中的值类型不能是内置类型的长形式（§[II.23.2.15](ii.23.2.15-methodspec.md)）。

<sup>2</sup> CLI的堆栈可以包含一个值类型。这些只能在堆栈上的特定值类型与所需的值类型完全匹配的情况下传递。

<sup>3</sup> 有特殊的指令来构造和传递一个`RefAny`。

<sup>4</sup> CLI 允许使用其内部的 `F` 类型传递浮点参数，参见 §[III.1.1.1](iii.1.1.1-numeric-data-types.md)。CIL 生成器当然可以包含一个显式的 `conv.r4`，`conv.r4.ovf` 或类似的指令。
...
方法的实现由一块连续的 CIL 指令提供，编码方式如下所述。方法的指令块的地址以及其长度在文件格式中有所指定（参见 [Partition II]()，CIL 物理布局）。第一条指令位于指令块的第一个字节（最低地址）。
...
直到指令块的结束，任何控制转移指令后面的指令都被解码为一条指令，因此即使它不是分支的目标，也参与定位后续的指令。只有指令可以出现在指令流中，即使是无法到达的。没有地址相对的数据寻址模式，原始数据不能直接嵌入到指令流中。某些指令允许将立即数据作为指令的一部分嵌入，然而这与允许在指令流中直接嵌入原始数据是不同的。无法到达的代码可以作为机器生成的代码的结果出现，并且是允许的，但它必须始终以正确形成的指令序列的形式出现。

指令流可以被翻译，相关的指令块在执行翻译之前被丢弃。因此，即使是捕获和操作代码地址的指令，如 `call`，`ret` 等，也可以被虚拟化，以便在翻译的地址上操作，而不是在 CIL 指令流的地址上操作。

#### 有效的分支目标

由指令流中识别的每条指令的第一个字节组成的地址集合定义了唯一有效的指令目标。指令目标包括分支指令中指定的分支目标，异常表中指定的目标，如保护范围（参见[Partition I]() 和 [Partition II]()），过滤器和处理器目标。

分支指令将分支目标指定为1字节或4字节的有符号相对偏移量；偏移量的大小由指令的操作码区分。偏移量被定义为相对于分支指令后面的字节。因此，偏移量为零的值目标是紧随其后的指令。 1 字节偏移量的值是通过将该字节解释为有符号的8位整数来计算的。

4 字节偏移量的值可以通过将字节连接成一个有符号的整数来计算，方式如下：最低地址的字节形成整数的最低有效字节，最高地址的字节形成整数的最高有效字节。这种表示法通常被称为 "小端字节序的有符号整数"。

#### 异常范围

异常表描述了由 `catch`，`fault` 或 `finally` 处理器保护的指令范围（参见 [Partition I]() 和 [Partition II]()）。受保护块，过滤子句或处理器的起始地址应为 §[III.1.7.2](iii.1.7.2-valid-branch-targets.md) 中指定的有效分支目标。如果受保护块，过滤子句或处理器在没有形成完整的最后一条指令的情况下结束，那么就是无效的。

#### 必须提供 maxstack

每个方法都指定了可以推送到 CIL 求值堆栈上的项目的最大数量。该值存储在 *`IMAGE_COR_ILMETHOD`* 结构中，该结构位于每个方法的 CIL 主体之前。一个方法如果指定的项目的最大数量小于方法的静态分析所需的数量（使用传统的控制流图，不分析数据），那么它就是无效的（因此也是不可验证的），并且不需要由 CLI 的符合规范的实现来支持。

Maxstack 与程序的分析相关，而不是与运行时堆栈的大小相关。它并没有指定堆栈帧的最大字节大小，而是指定了分析工具应该跟踪的项目的数量。
 
通过分析任何方法的 CIL 流，很容易确定将有多少项目被推送到CIL评估堆栈上。然而，提前指定最大数量有助于CIL到本地代码编译器（特别是只通过 CIL 流进行一次遍历的简单编译器）在分配模拟堆栈和/或验证算法的内部数据结构。

#### 后向分支约束

应该可以通过对任何方法的 CIL 指令流进行单次前向遍历，推断出每条指令处评估堆栈的确切状态（其中 "状态" 我们指的是评估堆栈上每个项目的数量和类型）。

特别地，如果该单次遍历分析到达一个指令，称之为位置 X，该指令紧随一个无条件分支之后，并且 X 不是早期分支指令的目标，那么显然，无法从现有信息推断出 X 处的评估堆栈状态。在这种情况下，CLI 要求 X 处的求值堆栈为空。

根据这个规则，如果后来的分支指令到 X 的求值堆栈非空，那么显然是无效的 CIL。

这个约束确保 CIL 代码可以被一个简单的 CIL 到本地代码编译器处理。它确保每个 CIL 开始时的评估堆栈状态可以从指令流的单次前向遍历分析中推断出来。

上述位置 X 处的堆栈状态可以通过各种方式推断：从前向分支到 X；因为X标记了异常处理器的开始，等等。

有关进一步信息，请参见以下内容：

 * 异常：[Partition I]()
 * 分支指令的验证条件：§[III.3](iii.3-base-instructions.md)
 * `tail.` 前缀：§[III.3.19](iii.3.19-call.md)

#### 分支验证约束

所有分支指令的目标应为该分支指令所在方法内的有效分支目标（参见§[III.1.7.2](iii.1.7.2-valid-branch-targets.md)）。

>---
### 可验证性和正确性

内存安全是一种属性，确保在同一地址空间运行的程序正确地彼此隔离（参见[Partition I]()）。因此，希望在运行程序之前测试程序是否具有内存安全性。不幸的是，可以证明，以100%的准确性做到这一点是不可能的。相反，CLI可以测试一个更强的限制，称为*可验证性*。每个被验证的程序都是内存安全的，但是一些不可验证的程序仍然是内存安全的。

正确的CIL是在所有符合CLI的实现上执行的CIL，其行为如本标准所指定的那样具有良好的定义。然而，正确的CIL不必在所有符合的实现中产生相同的行为；也就是说，行为可能是特定于实现的。

生成不可验证但已知内存安全的正确CIL代码是完全可以接受的，这是由编译器编写者知道的。因此，即使生成的编译器*知道*它是内存安全的，正确的CIL也可能不可验证。CIL指令的几个重要用途是不可验证的，例如`add`的指针算术版本，这是对C程序的忠实和高效编译所必需的。对于不可验证的代码，内存安全性是应用程序员的责任。

正确的CIL包含一个*可验证的子集*。可验证性描述给出了一个指令使用在CIL的可验证子集内的条件的详细信息。验证比CLI的基本功能所需的更详细地跟踪值的类型，因为它正在检查一个CIL代码序列是否不仅遵守CLI关于垃圾收集安全性的基本规则，而且还遵守CTS的类型规则。这有助于保证整个CLI的稳定运行。

每个操作描述的可验证性部分指定了正确的CIL生成和验证的要求。正确的CIL生成始终需要保证堆栈顶部的项目对应于堆栈转换图中显示的类型。可验证性部分只指定了在该图中未捕获的正确CIL生成的要求。验证测试既正确的CIL生成的要求，也测试与指令一起描述的特定验证条件。不满足CIL正确性要求的CIL序列的操作是未指定的。满足正确性要求的CIL序列的操作，但是不可验证的，可能会违反类型安全性，因此可能会违反安全性或内存访问约束。有关更多信息，请参见[II.3](ii.3-validation-and-verification.md)。

## III.1.8.1 可验证CIL的流控制限制

本小节指定了一个验证算法，该算法结合了关于单个CIL指令的信息（参见§[III.3](iii.3-base-instructions.md)）和元数据验证（参见Partition II），保证了内存完整性。

这里指定的算法为所有符合CLI的实现创建了一个最低级别，意味着任何被这个算法认为是可验证的程序都应被认为是可验证的，并且在所有符合CLI的实现上正确运行。

CLI提供了一个安全权限（参见[Partition IV]()），用于控制CLI是否应运行可能违反内存安全性的程序。任何根据这个标准可验证的程序都不会违反内存安全性，符合CLI的实现应运行这样的程序。实现也可能运行其他程序，只要它能够显示它们不违反内存安全性（通常是因为它们使用了一个利用关于实现的特定知识的验证算法）。

_[注：_ 虽然一个符合的实现需要接受并运行任何这个验证算法认为是可验证的程序，但可能有一些程序被一个给定的实现接受为可验证，但这个验证算法会认为它们不可验证。这样的程序将在给定的实现中运行，但不必被其他实现认为是可验证的。_

CLI的实现者被敦促提供一种测试方法，以确定他们的实现生成的程序是否满足这个可移植的可验证性标准。他们也被敦促指定他们的验证算法在哪些地方比这个标准更宽松。_结束注]_

只有有效的程序才能被验证。为了便于解释，这里描述的验证算法假设程序是有效的，并且并没有明确要求测试所有的有效性条件。有效性条件是在每个CIL指令的基础上指定的（参见§[III.3](iii.3-base-instructions.md)），并在[Partition II]()的整体文件格式中指定。

## III.1.8.1.1 验证算法

验证算法将尝试将一个有效的堆栈状态与每个CIL指令关联起来。堆栈状态指定了在代码的那一点上CIL堆栈上的插槽数量，以及每个插槽中应存在的所需类型。初始堆栈状态为空（堆栈上没有项目）。

验证假设CLI在将内存除了评估堆栈之外的所有部分对程序可见之前将其归零。CLI的符合规范的实现应提供这种可观察的行为。此外，可验证的方法应设置localsinit位，参见[Partition II (Flags for Method Headers)]()。如果没有设置这个位，那么CLI可能在访问局部变量的任何地方抛出一个_验证_异常，并且包含该方法的程序集没有被授予_安全权限_._跳过验证_。

_[理由：_ 这个要求强烈增强了程序的可移植性，一个众所周知的技术（确定分配分析）允许CIL到本地代码编译器最小化其性能影响。注意，CLI可能选择执行确定分配分析——在这种情况下，它可能确认一个方法，即使没有设置`localsinit`位，事实上可能是可验证的（因此不会抛出验证异常）_结束理由]_

_[注：_ 确定分配分析可以被CLI用来确定在读取之前哪些位置被写入。这样的位置不需要被归零，因为不可能观察到VES提供的内存的内容。

在C++实现（不需要确定分配分析）上的性能测量表明，添加这个要求几乎没有影响，即使在高度优化的代码中也是如此。此外，当这个归零没有执行时，客户错误地将错误归咎于编译器，因为这样的代码经常在对程序进行小的、无关的改变时失败。_结束注]_

验证算法将模拟代码中所有可能的控制流路径，并确保每个可达的CIL指令存在一个有效的堆栈状态。验证算法在模拟过程中不利用任何数据值（例如，它不执行常量传播），而只使用类型赋值。用于验证的类型系统的详细信息和用于合并堆栈状态的算法在§[III.1.8.1.3](iii.1.8.1.3-merging-stack-states.md)中提供。验证算法以以下方式终止：

 1. 成功，当所有的控制路径都被模拟。

 2. 不成功，当无法计算特定CIL指令的有效堆栈状态。

 3. 不成功，当本条款指定的其他测试失败。

除了无条件分支指令，`throw`，`rethrow`和`ret`，每个指令都有一个从每个指令到后续指令的控制流路径。每个分支指令（条件或无条件）也有一个到分支目标（或在`switch`指令的情况下，目标）的控制流路径。

验证模拟每个CIL指令的操作来计算新的堆栈状态，任何堆栈状态的指定条件（参见§[III.3](iii.3-base-instructions.md)）与模拟的堆栈状态之间的类型不匹配将导致验证算法失败。（注意，验证只模拟对堆栈状态的影响：它不执行实际的计算）。如果在下一个指令地址（对于条件分支或在**try**块内的指令，可能有多个这样的地址）存在一个不能与刚刚计算的堆栈状态合并的现有堆栈状态，那么算法也将失败。对于这个合并操作的规则，参见§[III.1.8.1.3](iii.1.8.1.3-merging-stack-states.md)。

CLI支持*受控可变性*管理指针的概念（参见§[III.1.8.1.2.2](iii.1.8.1.2.2-controlled-mutability-managed-pointers.md)，合并规则在§[III.1.8.1.3](iii.1.8.1.3-merging-stack-states.md)，`readonly.`指令前缀在§[III.2.3](iii.2.3-readonly.md)，`ldfld`指令在§[III.4.10](iii.4.10-ldfld.md)，`stfld`指令在§[III.4.28](iii.4.28-stfld.md)，以及`unbox`指令在§[III.4.32](iii.4.32-unbox.md)）。

VES确保特殊约束和类型约束都得到满足。这些约束可以在构造闭类型时尽早进行检查，也可以在调用受约束的泛型类型的方法、受约束的泛型方法、访问受约束的泛型类型中的字段，或创建受约束的泛型类型的实例时尽可能晚地进行检查。

为了适应泛型，类型兼容关系被扩展以处理：

 * 泛型参数：一个泛型参数只能*赋值给*（§[I.8.7.3](i.8.7.3-general-assignment-compatibility.md)）它自己。

 * 装箱的泛型参数：一个装箱的泛型参数可以*赋值给*（§[I.8.7.3](i.8.7.3-general-assignment-compatibility.md)）在泛型参数上声明的约束类型。

在验证语义中，将堆栈上的原始值或值类型的值装箱引入了类型为"装箱"类型的值；如果值类型是`Nullable<T>`（[Partition I.8.2.4]()），则引入了类型为"装箱"`T`的值。这种装箱类型的概念被扩展到泛型参数。将类型为泛型参数（例如`!0`）的值装箱在堆栈上引入了装箱参数类型的值（例如"装箱"`!0`）。值类型的装箱形式，以及现在的泛型参数的装箱形式，被用来支持在装箱值上进行有效的实例和虚方法调用。因为"装箱"类型在静态上记录了底层值的确切类型，所以没有必要在实例上执行一个从一些信息较少，但在语法上可以表达的引用类型的检查转换。

就像原始和非原始值类型的装箱形式一样，泛型参数的装箱形式只出现在验证堆栈上（在被`box`指令引入之后）。它们不能使用元数据签名显式地指定。

## III.1.8.1.2 验证类型系统

验证算法将逻辑上等价的类型进行压缩，因为它们不能导致内存安全性违规。验证算法使用的类型在§[III.1.8.1.2.1](iii.1.8.1.2.1-verification-types.md)中指定，类型兼容性规则在§[III.1.8.1.2.2](iii.1.8.1.2.2-controlled-mutability-managed-pointers.md)中指定，合并堆栈状态的规则在§[III.1.8.1.3](iii.1.8.1.3-merging-stack-states.md)中指定。

## III.1.8.1.2.1 验证类型

§[I.8.7](i.8.7-assignment-compatibility.md)指定了CLI和验证中使用的类型的映射。注意，验证将CLI类型压缩到一个更小的集合，该集合保持了这些类型在内存中的大小的信息，但然后再次压缩这些类型，以表示CLI堆栈将1，2和4字节的内置类型扩展到堆栈上的4字节类型的事实。同样，验证将堆栈上的浮点数视为64位量，而不考虑实际的表示。

数组是对象，但具有特殊的兼容性规则。

CLI有一个特殊的编码用于表示`null`，它代表一个已知为null值的对象，因此实际类型不确定。一个null值可能已知具有某种引用类型；例如，当它从局部变量或字段加载时；或者当它是由ldnull指令产生的结果时，具有特殊的null类型。null类型的null值只能存在于评估堆栈上。当任何指令的正确性或验证部分（§[III.1.8](iii.1.8-verifiability-and-correctness.md)）需要某种特定引用类型的值时，也允许使用null类型的值。如果提供了null类型的值并且指令对其进行了解引用，那么将抛出System.NullReferenceException；这在指令描述的适当异常区域中有所记载。

在下表中，"CLI类型"是在元数据中描述的类型。"验证类型"是在验证中用于类型兼容性规则的相应类型（参见§[I.8.7](i.8.7-assignment-compatibility.md)，*验证类型*，和§[III.1.8.1.2.2](iii.1.8.1.2.2-controlled-mutability-managed-pointers.md)），当考虑被调用的方法上的局部变量、参数和参数的类型时。"验证类型（在堆栈状态中）"对应于*中间类型*，§[I.8.7](i.8.7-assignment-compatibility.md)，并用于模拟将数据加载到堆栈的指令，并显示在验证算法的堆栈状态信息中实际维护的类型。"管理指针到类型"列显示了为管理指针跟踪的类型（参见§[I.8.7.2](i.8.7.2-assignment-compatibility-for-location-types.md)，*指针元素兼容*）。
...
一个方法可以被定义为返回一个管理指针，但是对这样的方法的调用是不可验证的。当通过引用返回时，验证是在返回站点进行的，而不是在调用站点进行的。

_[理由：_ 返回管理指针的一些用途是完全可验证的（例如，返回一个对象中的字段的引用）；但有些不是（例如，返回被调用方法的局部变量的指针）。在一般情况下跟踪这个是一个负担，因此没有包含在这个标准中。_结束理由]_

## III.1.8.1.2.2 受控可变性管理指针

`readonly.`前缀和`unbox`指令可以产生所谓的*受控可变性管理指针*。与普通的管理指针类型不同，受控可变性管理指针不能作为*验证器可分配给*（§[III.1.8.1.2.3](iii.1.8.1.2.3-verification-type-compatibility.md)）普通的管理指针；例如，它不能作为一个byref参数传递给一个方法。在控制流点，受控可变性管理指针可以与相同类型的管理指针合并，以产生受控可变性管理指针。
...
这个指针被称为受控可变性管理指针，因为定义类型决定了值是否可以被改变。对于没有公开任何公共字段或就地更新值的方法的值类，指针是只读的（因此得名前缀）。特别地，表示原始类型（如`System.Int32`）的类不公开变异器，因此是只读的。

## III.1.8.1.2.3 验证类型兼容性

验证类型兼容性是根据赋值兼容性定义的（参见§[I.8.7](i.8.7-assignment-compatibility.md)）。

如果且仅当`T`是`Q`的验证类型，`U`是`R`的验证类型，并且满足以下至少一项条件时，类型`Q`是*验证可分配给*`R`（有时写作`R` := `Q`）：

 1. `T`与`U`相同。_[注：_这是验证类型兼容性的自反性。_]_

 2. 存在某个`V`，使得`T`是*验证可分配给*`V`，并且`V`是*验证可分配给*`U`。_[注：_这是验证类型兼容性的传递性。_]_

 3. 根据§[I.8.7.3](i.8.7.3-general-assignment-compatibility.md)中的规则，`T`是*可分配给*`U`。

 4. `T`是类型`V`的受控可变性管理指针类型，`U`是类型`W`的受控可变性管理指针类型，`V`是*指针元素可分配给*`W`。

 5. `T`是管理指针类型`V&`，`U`是类型`W`的受控可变性管理指针类型，`V`是*指针元素可分配给*`W`。
...
_[注：_验证可分配给扩展了可分配给，以处理只能在堆栈上出现的类型，即装箱类型、受控可变性管理指针类型和null类型。_结束注]_

在[Partition III]()的其余部分，有时使用符号"`U` := `T`"表示`T`是*验证可分配给*`U`。

## III.1.8.1.3 合并堆栈状态

当验证算法模拟所有控制流路径时，它应将模拟的堆栈状态与流中下一个CIL指令的任何现有堆栈状态合并。如果没有现有的堆栈状态，模拟的堆栈状态将被存储以供将来使用。否则，合并应按照以下方式计算，并存储以替换CIL指令的现有堆栈状态。如果合并失败，验证算法应失败。

合并应通过比较每个堆栈状态中的插槽数量来计算。如果它们不同，合并应失败。如果它们匹配，那么整体合并应通过按以下方式逐插槽合并状态来计算。设`T`是新计算状态的插槽上的类型，S是先前存储状态的相应插槽上的类型。合并的类型，`U`，应按照以下方式计算（回想一下，`S` := `T`是在§[III.1.8.1.2.2](iii.1.8.1.2.2-controlled-mutability-managed-pointers.md)中定义的兼容性函数）：
...
将受控可变性管理指针与同一类型的普通（即非受控可变性）管理指针合并，结果是该类型的受控可变性管理指针。

## III.1.8.1.4 类和对象初始化规则

VES确保所有静态变量最初都被归零（即，内置类型为0或false，对象引用为null），因此验证算法不测试静态变量的确定分配。

除非已经在新构造的对象上调用了基类的构造函数或对象类的不同构造函数，否则对象构造函数不应返回。除非已经调用了基类构造函数，否则验证算法应将**this**指针视为未初始化。除了存储到对象的字段和从对象的字段加载之外，不能对未初始化的**this**执行任何操作。

如果构造函数产生异常，则对应catch块中的this指针仍未初始化。

## III.1.8.1.5 委托构造

委托构造的验证是基于代码序列而不是单个指令。这些详细信息在`newobj`指令的描述（§[III.4.21](iii.4.21-newobj.md)）中有详细说明。

如果分支目标在这些指令序列中（除了在序列的开始处），则验证算法应失败。

_[注：_ 请参见[Partition II]()，了解委托的签名以及关于在构造函数中使用的方法的签名以及委托类上的Invoke和其他方法的签名的有效性要求。_结束注]_

## III.1.9 元数据令牌

许多CIL指令后面都跟着一个"元数据令牌"。这是一个4字节的值，它指定了元数据表中的一行，或者User String堆中的起始字节偏移量。令牌的最高有效字节指定了表或堆。例如，值0x02指定了_TypeDef_表；值0x70指定了User String堆。该值对应于分配给该元数据表的编号（参见[Partition II]()，了解完整的表列表）或者对应于User String堆的0x70。最低有效的3字节指定了元数据表中的目标行，或者User String堆中的起始字节偏移量。元数据表中的行是从1开始编号的，而堆中的偏移量是从0开始编号的。（所以，例如，元数据令牌的值0x02000007指定了_TypeDef_表中的行号7）

## III.1.10 异常抛出

一个CIL指令可以抛出一系列的异常。CLI也可以抛出一个称为`ExecutionEngineException`的通用异常。详情请参见[Partition I]()。

## III.2 指令前缀

这些特殊的值被保留用于在特定指令之前。它们本身并不构成完整的指令。在前缀后面的指令上进行分支是无效的CIL，但是前缀本身是一个有效的分支目标。如果一个前缀后面没有立即跟随它允许前置的指令，那么它就不是有效的CIL。

## III.2.1 constrained. - （前缀）在变量类型的值上调用成员

 格式 | 汇编格式 | 描述 |
 ---- | ---- | ----
 FE 16 _\<T\>_ | `constrained.` _thisType_ | 在类型被约束为类型`T`的类型上调用虚方法
...
`constrained.`前缀只允许在`callvirt`指令上。ptr的类型必须是_thisType_的管理指针（`&`）。constrained前缀的设计是为了允许以独立于_thisType_是值类型还是引用类型的统一方式进行`callvirt`指令。

当`callvirt`方法指令被`constrained.` _thisType_前缀修饰时，指令按以下方式执行。

如果_thisType_是引用类型（而不是值类型），那么

 * _ptr_被解引用并作为**this**指针传递给`callvirt`的方法

如果_thisType_是值类型并且_thisType_实现了_method_，那么

 * _ptr_不修改地作为**this**指针传递给_thisType_实现的_method_的调用

如果_thisType_是值类型并且_thisType_没有实现_method_，那么

 * _ptr_被解引用，装箱，并作为**this**指针传递给`callvirt`的方法

这最后一种情况只能在_method_被定义在`System.Object`，`System.ValueType`，或`System.Enum`上，并且没有被_thisType_重写时发生。在这最后一种情况下，装箱导致原始对象的一个副本被创建，然而由于所有在`System.Object`，`System.ValueType`，和`System.Enum`上的方法都不修改对象的状态，这个事实不能被检测到。
...
受限前缀将紧接着被`callvirt`指令跟随。_thisType_应该是一个有效的`typedef`，`typeref`，或`typespec`元数据令牌。

#### 可验证性：

_ptr_参数将是一个管理指针（`&`）到_thisType_。此外，在上述_ptr_转换之后，`callvirt`指令的所有正常验证规则都适用。这相当于要求一个装箱的_thisType_必须是方法所属类的子类。

_[理由：_ 这个指令的目标是实现虚函数调用的一致性，因此这样的调用可以在泛型例程中进行验证。实现这种一致性的一种方式是在进行`callvirt`之前总是对**this**指针进行装箱。这对于引用类型（其中`box`是无操作）和值类型都是有效的。这种方法的问题在于，在值类型的情况下会产生一个副本。因此，如果被调用的方法修改了值类型的状态，那么在调用完成后这种修改将不会反映出来，因为这种修改是在装箱的副本中进行的。这种语义差异（以及额外装箱的性能成本），使得这种替代方案无法接受。_结束理由]_

## III.2.2 no. – (prefix) 可能跳过故障检查

 格式 | 汇编格式 | 描述
 ---- | ---- | ----
 FE 19 _\<unsigned int8\>_ | `no.` \{ `typecheck` \| `rangecheck` \| `nullcheck` \} | 在执行后续指令的过程中，通常执行的指定故障检查可以/应该被跳过。

#### 描述：

这个前缀表示在执行后续指令时不需要执行指定的故障检查。跟在指令代码后面的字节指示哪些检查可以选择性地被跳过。这个指令是不可验证的。

前缀可以在以下情况下使用：

 * 0x01: `typecheck` (`castclass`, `unbox`, `ldelema`, `stelem`, `stelem`)。CLI可以选择性地跳过在执行后续指令的过程中通常执行的任何类型检查。如果检查失败，`InvalidCastException`可以选择性地仍然被抛出。

 * 0x02: `rangecheck` (`ldelem.*`, `ldelema`, `stelem.*`)。CLI可以选择性地跳过在执行后续指令的过程中通常执行的任何数组范围检查。如果检查失败，`IndexOutOfRangeException`可以选择性地仍然被抛出。
 
  * 0x04: `nullcheck` (`ldfld`, `stfld`, `callvirt`, `ldvirtftn`, `ldelem.*`, `stelem.*`, `ldelema`)。CLI可以选择性地跳过在执行后续指令的过程中通常执行的任何空引用检查。如果检查失败，`NullReferenceException`可以选择性地仍然被抛出。

字节值可以被OR-ed；例如，值0x05表示`typecheck`和`nullcheck`都可以选择性地被省略。
...
### III.2.3 readonly. (prefix) &mdash; 后续指令返回受控可变性管理指针

 格式 | 汇编格式 | 描述
 ---- | ---- | ----
 FE 1E | `readonly.` | 指定后续数组地址操作在运行时不执行类型检查，并且它返回一个受控可变性管理指针

#### 描述：

此前缀只能紧接在`ldelema`指令和对数组的特殊*Address*方法的调用之前出现。它对后续操作的影响有两方面。

1. 在运行时，不执行类型检查操作。（对于值类的情况，从来没有运行时检查，所以这是一个无操作）。

 2. 验证器将地址操作的结果视为受控可变性管理指针（§[III.1.8.1.2.2](iii.1.8.1.2.2-controlled-mutability-managed-pointers.md)）。
...
受控可变性管理指针必须遵守在§[III.1.8.1.2.2](iii.1.8.1.2.2-controlled-mutability-managed-pointers.md)的(2)中给出的验证规则。另请参见§[III.1.8.1.3](iii.1.8.1.3-merging-stack-states.md)。

_[理由：_ `readonly.`前缀的主要目标是在泛型代码中从数组中获取元素时避免类型检查。例如，表达式

 _array_[_i_]._method_()

其中_array_的类型为`T[]`（其中`T`是一个泛型参数），并且`T`被约束为具有方法_method_的接口，可能会编译成以下IL代码。
...
如果没有`readonly.`前缀，`ldelema`在`!0`是引用类的情况下会进行类型检查。这种类型检查不仅效率低下，而且在语义上是不正确的。`ldelema`的类型检查进行了精确匹配类型检查，这在一般情况下过于强大。如果数组中包含了`!0`的派生类，那么上面的代码将无法通过`ldelema`类型检查。我们获取数组元素的地址而不是元素本身（这是源代码所说的）的唯一原因，是因为我们需要一个对_value_[_i_]的句柄，这个句柄对于值类型和引用类型都可以传递给受约束的`callvirt`指令。

如果数组包含引用类型的元素，一般来说，跳过运行时检查是不安全的。为了安全，我们必须确保不通过这个指针修改数组。上述的验证规则确保了这一点。由于我们明确允许只读指针作为实例方法调用的对象传递，这些指针对于值类型并不严格地只读，但是对于值类型没有类型安全问题。_结束理由]_

### III.2.4 tail. (prefix) &mdash; 调用终止当前方法
...
`tail.`前缀应立即在`call`，`calli`，或`callvirt`指令之前。它表示当前方法的堆栈帧不再需要，因此可以在执行调用指令之前将其移除。因为调用返回的值将是此方法返回的值，所以调用可以转换为跨方法跳转。

评估堆栈应为空，除了由后续调用传递的参数。调用指令后面的指令应该是`ret`。因此，唯一有效的代码序列是

 `tail.` `call` (or `calli` or `callvirt`) *somewhere*  
 `ret`

正确的CIL不应分支到`call`指令，但是允许分支到`ret`。堆栈上的唯一值应该是被调用的方法的参数。

`tail.` `call`（或`calli`或`callvirt`）指令不能用于从try，filter，catch或finally块中转移控制。参见[Partition I]()。

当从不受信任的代码转移到受信任的代码时，不能丢弃当前帧，因为这将危及代码身份安全。因此，安全检查可能会导致忽略`tail.`，留下一个标准的调用指令。

同样，为了允许在调用返回后退出同步区域，当用于退出标记为同步的方法时，将忽略`tail.`前缀。

还可能存在一些实现特定的限制，防止在某些情况下遵守`tail.`前缀。虽然实现可以在这些情况下自由地忽略`tail.`前缀，但是它们应该被清楚地记录下来，因为它们可能会影响程序的行为。

CLI实现需要尊重`tail.`调用请求，其中调用者和被调用者方法可以静态地确定位于同一程序集中；并且调用者不在同步区域中；并且调用者和被调用者满足下面的"可验证性"规则中列出的所有条件。（"尊重"`tail.`前缀意味着移除调用者的帧，而不是恢复到常规的调用序列）。因此，CLI实现不需要尊重`tail.` `calli`或`tail.` `callvirt`序列。

_[理由：_ `tail.`调用允许一些线性空间算法转换为常量空间算法，并且某些语言需要它。在存在`ldloca`和`ldarga`指令的情况下，从CIL到本地代码的编译器并不总是能够最优地确定何时可以自动插入tail。_结束理由]_
...
正确的CIL遵守上述的控制转移约束。此外，如果它们指向即将被移除的堆栈帧，那么不能将管理指针传递给被调用的方法。被调用的方法的返回类型应该是*可分配给*（§[I.8.7.3](i.8.7.3-general-assignment-compatibility.md)）当前方法的返回类型。

#### 可验证性：

验证要求不将管理指针传递给被调用的方法，因为它不跟踪指向当前帧的指针。

## III.2.5 unaligned. (prefix) &ndash; 指针指令可能不对齐
...
`unaligned.`前缀指定堆栈上的*addr*（一个非托管指针（`&`）或`native int`）可能不对齐到紧接着的`ldind`，`stind`，`ldfld`，`stfld`，`ldobj`，`stobj`，`initblk`，或`cpblk`指令的自然大小。也就是说，对于`ldind.i4`指令，*addr*的对齐可能不是到4字节边界。对于`initblk`和`cpblk`，默认对齐是依赖于架构的（32位CPU上是4字节，64位CPU上是8字节）。不将其输出限制为32位字大小的代码生成器（参见[Partition I]()和[Partition II]())应该在编译时不能确定对齐为8字节时使用`unaligned.`。

*alignment*的值应为1，2或4，表示生成的代码应假定*addr*分别对齐到字节，双字节或四字节。

_[理由：_ 虽然`cpblk`指令的对齐逻辑上需要两个数字（一个用于源，一个用于目标），但是如果只指定较低的数字，对性能的影响并不明显。_结束理由]_

`unaligned.`和`volatile.`前缀可以以任何顺序组合。它们应立即在`ldind`，`stind`，`ldfld`，`stfld`，`ldobj`，`stobj`，`initblk`，或`cpblk`指令之前。

_[注：_ 有关原子性和数据对齐的信息，请参见 [Partition I, 12.7]()。_结束注]_
...
`unaligned.`前缀应立即跟随上述指令之一。

## III.2.6 volatile. (prefix) &ndash; 指针引用是易变的
...
`volatile.`前缀指定*addr*是一个易变地址（即，它可以被当前执行线程外部引用），并且不能缓存读取该位置的结果，或者不能抑制对该位置的多次存储。将访问标记为`volatile.`只影响该单个访问；对同一位置的其他访问应分别标记。不需要原子地访问易变位置。（参见 [Partition I, "Memory Model and Optimizations"]()）

`unaligned.`和`volatile.`前缀可以以任何顺序组合。它们应立即在`ldind`，`stind`，`ldfld`，`stfld`，`ldobj`，`stobj`，`initblk`，或`cpblk`指令之前。只有`volatile.`前缀允许与`ldsfld`和`stsfld`指令一起使用。
...
`volatile.`前缀应立即跟随上述指令之一。

## III.3 基本指令

这些指令形成了一套"图灵完备"的基本操作。它们独立于可能使用的对象模型。与CTS的对象模型具体相关的操作包含在对象模型指令部分。

## III.3.1 add &ndash; 添加数值
...
`add`指令将_value2_添加到_value1_并将结果推入堆栈。对于整数操作，不检测溢出（但请参见`add.ovf`）；浮点溢出返回`+inf`或`-inf`。

可接受的操作数类型及其相应的结果数据类型包含在[表2：二进制数值操作]()中。
...
## III.3.2 add.ovf.\<signed\> &ndash; 添加整数值并检查溢出
...
`add.ovf`指令将_value1_和_value2_相加并将结果推入堆栈。可接受的操作数类型及其相应的结果数据类型包含在[表7：溢出算术操作]()中。
...
`and`指令计算_value1_和_value2_的按位与，并将结果推入堆栈。可接受的操作数类型及其相应的结果数据类型包含在[表5：整数操作]()中。
